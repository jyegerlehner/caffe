...
template <typename Dtype>
class DiffMagnitudeMonitoringLayer: public Layer<Dtype> {
 public:
  explicit DiffMagnitudeMonitoringLayer(const LayerParameter& param)
     : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
     const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "DiffMagnitudeMonitoring"; }
